...
叠加实验：3 epoch训练（ood_final）+ 浅层重置（reset_10_qv）
验证两种方法是否可以叠加产生更强的OOD提升

对比：
  base             : math=0.720 spatial=0.360 planning=0.520
  v2（1 epoch）    : math=0.700 spatial=0.360 planning=0.540
  reset_10_qv      : math=0.740 spatial=0.380 planning=0.560  (+0.020)
  ood_final（3ep） : math=0.760 spatial=0.340 planning=0.540  (+0.013)
  ood_final+reset  : ???
"""
...
CONFIG = {
    ...
    "adapter_path":    "./checkpoints/qwen-gomoku-ood-monitor/final_model",
    "output_dir":      "./results/evaluations/combined_reset",
    "max_new_tokens":  80,
    "reset_layers":    10,
    "reset_modules":   ["q_proj", "v_proj"],
}
...
PREV_RESULTS = {
    "v2":          {"math": 0.700, "spatial": 0.360, "planning": 0.540, "logic": 1.000},
    "reset_10_qv": {"math": 0.740, "spatial": 0.380, "planning": 0.560, "logic": 1.000},
    "ood_final":   {"math": 0.760, "spatial": 0.340, "planning": 0.540, "logic": 1.000},
}
...
def main():
    print("=" * 65)
    print("叠加实验：ood_final（3 epoch）+ reset_10_qv（前10层清零）")
    print("=" * 65)

    cache_file = os.path.join(CONFIG["output_dir"], "ood_final_reset.json")
    if os.path.exists(cache_file):
        ...
        for name, param in model.named_parameters():
            if ('lora_A' in name or 'lora_B' in name):
                if any(mod in name for mod in CONFIG["reset_modules"]):
                    m = re.search(r'\.(\d+)\.', name)
                    if m and int(m.group(1)) < CONFIG["reset_layers"]:
                        param.data.zero_()
                        reset_count += 1
        print(f"  清零了{reset_count}个LoRA参数（前{CONFIG['reset_layers']}层"
              f"{CONFIG['reset_modules']}）")

        model.eval()
        result = evaluate(model, tokenizer, device)
    ...
    print("完整实验对比（所有方法）")
    print("=" * 70)
    print(f"\n{'模型':<20} {'math':>6} {'spatial':>8} {'planning':>9} "
          f"{'logic':>6} {'avg_delta':>10}")
    print("-" * 65)

    all_models = {
        "base":               BASE_RESULTS,
        "v2（1 epoch）":      PREV_RESULTS["v2"],
        "reset_10_qv":        PREV_RESULTS["reset_10_qv"],
        "ood_final（3ep）":   PREV_RESULTS["ood_final"],
        "ood_final+reset":    result,
    }
    ...
    for name, res in all_models.items():
        ...
        tag = "← 基准" if name == "base" else f"{avg_d:>+.3f}"
        print(f"{name:<20} {vals}  {tag}")
    ...
    for task in tasks:
        v_reset  = PREV_RESULTS["reset_10_qv"][task]
        v_final  = PREV_RESULTS["ood_final"][task]
        v_comb   = result[task]
        v_base   = BASE_RESULTS[task]
        # 如果叠加有协同效应，combined应该 > max(reset, final)
        expected_max = max(v_reset, v_final)
        synergy = v_comb - expected_max
        print(f"  {task:<10}: reset={v_reset:.3f}  final={v_final:.3f}  "
              f"combined={v_comb:.3f}  vs_base={v_comb-v_base:+.3f}  "
              f"协同效应={synergy:+.3f}")

    # 综合结论
    combined_avg = np.mean([result[t] - BASE_RESULTS[t] for t in tasks])
    best_single = max(
        np.mean([PREV_RESULTS["reset_10_qv"][t] - BASE_RESULTS[t] for t in tasks]),
        np.mean([PREV_RESULTS["ood_final"][t] - BASE_RESULTS[t] for t in tasks]),
    )

    print(f"\n【结论】")
    print(f"  combined avg_delta = {combined_avg:+.3f}")
    print(f"  best single method = {best_single:+.3f}")
    if combined_avg > best_single + 0.005:
        print(f"  ✅ 叠加有协同效应！combined优于任何单一方法")
        print(f"  两种方法作用于不同层面（训练时长 vs 后处理），可以叠加")
    elif combined_avg > best_single - 0.005:
        print(f"  = 叠加效果与最优单一方法持平")
        print(f"  两种方法部分重叠，叠加无额外收益")
    else:
        print(f"  ✗ 叠加反而下降，两种方法存在相互干扰")

    with open(os.path.join(CONFIG["output_dir"], "combined_results.json"),
              "w", encoding="utf-8") as f:
        json.dump({
            "result": result,
            "combined_avg_delta": float(combined_avg),
            "best_single_delta": float(best_single),
        }, f, indent=2)
    print(f"\n✅ 结果保存至: {CONFIG['output_dir']}/combined_results.json")
