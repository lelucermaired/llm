"""
shallow_reset_eval.py

浅层重置实验：将v2模型前N层的LoRA权重清零
理论依据：Jin et al. (2025) arXiv:2509.12235
  重置早期层（前20%）可恢复70-80%的OOD损失

实验设计：
  reset_0:  不重置（纯v2）
  reset_4:  清零前4层LoRA（约14%）
  reset_7:  清零前7层LoRA（约25%）
  reset_10: 清零前10层LoRA（约36%）
  reset_14: 清零前14层LoRA（约50%）
"""
...
CONFIG = {
    ...
    "output_dir":      "./results/evaluations/shallow_reset",
    "reset_layers":    [0, 4, 7, 10, 14],  # 清零前N层
    "max_new_tokens":  80,
}
...
def main():
    print("=" * 65)
    print("浅层重置实验")
    print("将v2前N层LoRA权重清零，观察OOD性能变化")
    print("理论依据：Jin et al. (2025) 重置前20%层可恢复70-80% OOD")
    print("=" * 65)
    ...
    for n_reset in CONFIG["reset_layers"]:
        cache_file = os.path.join(CONFIG["output_dir"], f"reset_{n_reset}.json")
        if os.path.exists(cache_file):
            with open(cache_file, encoding="utf-8") as f:
                res = json.load(f)
            all_results[n_reset] = res
            print(f"[缓存] reset_{n_reset}: math={res['math']:.3f} "
                  f"spatial={res['spatial']:.3f}")
            continue

        print(f"\n{'='*50}")
        print(f"加载模型：前{n_reset}层LoRA清零")
        ...
        for name, param in model.named_parameters():
            if 'lora_A' in name or 'lora_B' in name:
                ...
                if layer_match:
                    ...
                    if layer_idx < n_reset:
                        param.data.zero_()
                        reset_count += 1

        print(f"  清零了 {reset_count} 个LoRA参数张量（前{n_reset}层）")
        model.eval()

        res = evaluate(model, tokenizer, device)
        all_results[n_reset] = res

        print(f"  ✅ reset_{n_reset}: math={res['math']:.3f} "
              f"spatial={res['spatial']:.3f} "
    ...
    print(f"\n{'重置层数':>8}  {'math':>8} {'spatial':>8} {'planning':>8} {'logic':>8}  {'avg_delta':>10}  说明")
    print("-" * 75)

    for n_reset in sorted(all_results.keys()):
        res = all_results[n_reset]
        deltas = [res[t] - BASE_RESULTS[t] for t in tasks]
        avg_delta = np.mean(deltas)
        vals = " ".join(f"{res[t]:>8.3f}" for t in tasks)
        pct = n_reset / 28 * 100
        tag = "← base(无重置)" if n_reset == 0 else f"前{pct:.0f}%层清零"
        print(f"{n_reset:>8}  {vals}  {avg_delta:>+10.3f}  {tag}")
    ...
    # 找最优重置层数
    best_n = max(all_results.keys(),
                 key=lambda n: np.mean([all_results[n][t] - BASE_RESULTS[t]
                                        for t in tasks]))
    best_avg = np.mean([all_results[best_n][t] - BASE_RESULTS[t] for t in tasks])

    print(f"\n【结论】")
    if best_avg > 0.01 and best_n > 0:
        print(f"  最优重置层数：前{best_n}层，avg_delta={best_avg:+.3f}")
        print(f"  浅层重置有效恢复了部分OOD泛化能力")
        print(f"  支持Jin et al.(2025)：重置早期层可恢复OOD损失")
    elif best_n == 0:
        print(f"  不重置（纯v2）性能最好，浅层重置反而有损害")
        print(f"  说明v2的LoRA在浅层也编码了有用信息")
    else:
        print(f"  所有重置策略与base持平（avg_delta≈0）")
        print(f"  浅层重置对本实验无显著效果")

    save_path = os.path.join(CONFIG["output_dir"], "reset_results.json")
    with open(save_path, "w", encoding="utf-8") as f:
        json.dump({str(n): r for n, r in all_results.items()},
                  f, indent=2)
    print(f"\n✅ 结果保存至: {save_path}")
