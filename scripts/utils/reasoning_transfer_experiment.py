...
class ExperimentConfig:
    ...
    chess_finetuned_path: str = "./checkpoints/qwen-gomoku-merged/final_model"
    ...
    # 测试配置
    n_samples_per_test: int = 50  # 每个测试集的样本数
    ...
    def __post_init__(self):
        if self.math_benchmarks is None:
            self.math_benchmarks = []

        if self.logic_benchmarks is None:
            self.logic_benchmarks = ["logiqa"]
...
class ReasoningBenchmarkEvaluator:
    ...
    def _evaluate_logiqa(self) -> Dict[str, Any]:
        try:
            import gc
            dataset = load_dataset("tasksource/logiqa-2.0-nli",split="test")
            n_samples = min(self.config.n_samples_per_test, len(dataset))
            ...
            labels = [
                "Yes" if ex["label"] == 1 else "No"
                for ex in sampled_data
            ]
            options_list = [ex["options"] for ex in sampled_data]
            answers = [str(ex["correct_option"]) for ex in sampled_data]
            labels = ["A", "B", "C", "D"]

            def make_prompt(q, options):
                options_text = "\n".join([f"{labels[i]}. {options[i]}" for i in range(len(options))])
                return f"""Answer the following logical reasoning question by choosing the best option.
    ...
    Answer with only the letter:"""
            # 第一轮：基础模型
            print("加载基础模型...")
            self.mm.base_model = AutoModelForCausalLM.from_pretrained(
                self.mm.config.base_model_path,
                torch_dtype=torch.float16,
                trust_remote_code=True,
            ).to(self.config.device)

            self.mm.base_model.eval()

            base_scores = []
            for i, q in enumerate(tqdm(questions, desc="ARC基础模型")):
                prompt = make_prompt(q, options_list[i], labels_list[i])
                resp = self.mm.generate_response(self.mm.base_model, prompt, max_new_tokens=10)
                ans = self._extract_option_letter(resp)
                base_scores.append(ans == answers[i])

            # 释放基础模型
            del self.mm.base_model
            self.mm.base_model = None
            ...
            base_for_chess = AutoModelForCausalLM.from_pretrained(
                self.mm.config.base_model_path,
                torch_dtype=torch.float16,
                trust_remote_code=True,
            ...
            self.mm.chess_model = PeftModel.from_pretrained(
                base_for_chess,
                self.mm.config.chess_finetuned_path,
            )

            self.mm.chess_model.to(self.config.device)

            chess_scores = []
            for i, q in enumerate(tqdm(questions, desc="ARC棋类模型")):
                prompt = make_prompt(q, options_list[i], labels_list[i])
                resp = self.mm.generate_response(self.mm.chess_model, prompt, max_new_tokens=10)
                ans = self._extract_option_letter(resp)
                chess_scores.append(ans == answers[i])

            # 释放chess模型
            del self.mm.chess_model
            self.mm.chess_model = None
            torch.cuda.empty_cache()
            gc.collect()
    ...
    def _extract_option_letter(self, text: str) -> str:
        ...
        # 匹配 "Answer: A" 或 "The answer is B" 等格式
        patterns = [
            r"(?:answer is|answer:|the answer)\s*:?\s*([ABCD])",
            r"^([ABCD])[.\):]",
            r"\b([ABCD])\b",
        ]
    ...
    def _evaluate_gsm8k(self) -> Dict[str, Any]:
        try:
            ...
            ground_truths = [ex["answer"] for ex in sampled_data]
            true_answers = []
            for gt in ground_truths:
                m = re.search(r"####\s*(-?\d+\.?\d*)", gt)
                true_answers.append(float(m.group(1)) if m else None)

            def make_prompt(q):
                return f"""Solve the following math problem. At the end, write your final answer as: The answer is [number].
    ...
    Answer:"""
            ...
            base_scores = []
            for i, q in enumerate(tqdm(questions, desc="GSM8K基础模型")):
                resp = self.mm.generate_response(self.mm.base_model, make_prompt(q), max_new_tokens=256)
                ans = self._extract_numeric_answer(resp)
                base_scores.append(self._compare_answers(ans, true_answers[i]))
            ...
            chess_model = PeftModel.from_pretrained(
                base_for_chess,
                self.mm.config.chess_finetuned_path,
            )

            chess_model = chess_model.merge_and_unload()
            chess_model.to(self.config.device)
            chess_model.eval()

            self.mm.chess_model = chess_model

            chess_scores = []
            cot_scores = []
            for i, q in enumerate(tqdm(questions, desc="GSM8K棋类模型")):
                resp = self.mm.generate_response(self.mm.chess_model, make_prompt(q), max_new_tokens=256)
                ans = self._extract_numeric_answer(resp)
                chess_scores.append(self._compare_answers(ans, true_answers[i]))
                cot_scores.append(self._cot_structure_score(resp))
            ...
            results = self._calculate_results(base_scores, chess_scores, [])
...
请选择正确选项（A, B, C, D）："""
                ...
                base_response = self.mm.generate_response(self.mm.base_model, prompt, max_new_tokens=256)
                chess_response = self.mm.generate_response(self.mm.chess_model, prompt, max_new_tokens=256)
...
请给出达到目标状态所需的操作序列："""
    ...
    def _extract_numeric_answer(self, text: str):
        ...
        # 3️⃣ 匹配 answer is
        match = re.search(r"(?:answer is|the answer is)\s*(-?\d+\.?\d*)", text, re.IGNORECASE)
        if match:
            return float(match.group(1))
...
def main():
    ...
    config = ExperimentConfig(
        ...
        chess_finetuned_path="./checkpoints/qwen-gomoku-v2/final_model",
        results_dir="./experiment_results_logiqa",
