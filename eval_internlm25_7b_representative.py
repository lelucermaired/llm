...
DEFAULT_BASE_MODEL = "/root/autodl-tmp/models/internlm2_5-7b-chat"
DEFAULT_ADAPTER = "./checkpoints/internlm25-7b-gomoku-cot-maxlora/final_model"
DEFAULT_CACHE_DIR = "/root/autodl-tmp/hf_cache"
DEFAULT_OUTPUT = "./results/cross_model/internlm25_7b_gomoku_cot_representative.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate InternLM2.5-7B BASE vs Gomoku CoT adapter on representative BBH tasks.")
...
def gold_answer(example):
    return re.sub(r"[()]", "", str(example["target"]).strip()).strip()
